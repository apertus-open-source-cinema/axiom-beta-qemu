...
sfdisk -u S -L -S 16 IMAGE.dd << EOF
...
wget -c http://archlinuxarm.org/os/ArchLinuxARM-zedboard-latest.tar.gz
...
(cd linux-xlnx.git; sudo make CROSS_COMPILE=$CROSS ARCH=$ARCH INSTALL_MOD_PATH=../ROOT.fs modules_install )
...
sudo mkfs.ext4 -d ROOT.fs ROOT.part $[ BLKROOT / 2 ]
